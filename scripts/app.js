...
myApp.filter('formatText', function () {
    return function (input) {
        ...
        return '<h3>' + text[0] + '</h3><p>' + toTitleCase(description).trim().replace(' ', ': ').replace('Torn: Down', 'Torn Down:').replace("Circa", "circa").replace('Built: Mid—1950s', 'Built: mid—1950s').replace('Purchased: Early 1960s', 'Purchased: early 1960s').replace('Built: 2000, 2002, And 2007', 'Built: 2000, 2002, and 2007') + '</p>';
    }
});
			...
			if(nextClicked){
			 ...
			 p.fill(255,255,255,0);
              ...
              var selectedName = names[selected].indexOf('-') < names[selected].length - 1 ?  names[selected] : names[selected].replace('-', '');
				   		p.text(selectedName, (hitParams[selected][0] + 1) -100 , hitParams[selected][1] + 1);
				   		p.text(selectedName, (hitParams[selected][0] + 1) -100, hitParams[selected][1]);
				   		p.text(selectedName, (hitParams[selected][0]) -100, hitParams[selected][1] + 1);
				   		p.text(selectedName, (hitParams[selected][0] - 1) -100, hitParams[selected][1] - 1);
				   		p.fill(255);
				   		p.text(selectedName, hitParams[selected][0] - 100, hitParams[selected][1]);
			   		}
